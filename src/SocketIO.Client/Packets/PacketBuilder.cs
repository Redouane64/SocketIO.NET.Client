...
/// <summary>
///     Builds a Socket.IO packet to send. see: https://socket.io/docs/v4/socket-io-protocol
/// </summary>
/// <remarks>
///     <para>
...
///     </para>
///     <para>
///         The counterpart of <see cref="Packet" />, which reads the arguments that
///         arrived. Two types rather than one because the two sets of operations are
///         never both valid: there is nothing to read on a packet being built, and
///         nothing to add to one that came off the wire.
///     </para>
/// </remarks>
public sealed class PacketBuilder
{
    ...
    public static readonly PacketBuilder Connect = new(PacketType.Connect);

    public static readonly PacketBuilder Disconnect = new(PacketType.Disconnect);
    ...
    public PacketBuilder(PacketType type, string? @namespace)
        : this(type, @namespace, null, null)
    {
    }

    public PacketBuilder(PacketType type, string? @namespace, string? @event)
        : this(type, @namespace, @event, null)
    {
    }

    public PacketBuilder(PacketType type, int ackId, string? @namespace, string? @event)
    ...
    /// </summary>
    private PacketBuilder(PacketType type, string? @namespace, string? @event, int? ackId)
    {
    ...
    ///     instance — <see cref="Connect" /> among them — can be sent repeatedly.
    /// </remarks>
    internal void Serialize(IBufferWriter<byte> writer)
    {
        ...
        if (Type.CarriesAttachments() && _attachments.Count == 0)
        {
            throw new InvalidOperationException(
    ...
    private void WriteHeader(IBufferWriter<byte> writer)
    {
        ...
        if (!string.Equals(Namespace, Packet.DefaultNamespace, StringComparison.Ordinal))
        {
    ...
    /// </summary>
    internal static string NormalizeNamespace(string? @namespace)
    {
        if (string.IsNullOrWhiteSpace(@namespace))
        {
            return Packet.DefaultNamespace;
        }
